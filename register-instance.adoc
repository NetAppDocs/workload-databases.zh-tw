...
註冊執行個體（SQL Server）或資料庫（Oracle）涉及三個步驟：執行個體或資料庫驗證、FSx for ONTAP 驗證以及準備工作。準備工作包括在執行個體或資料庫上安裝 AWS、NetApp 和 PowerShell 模組，並符合 Workload Factory for Databases 功能（例如 link:analyze-error-logs.html["錯誤日誌分析"] 或 link:optimize-configurations.html["架構完善的評估"]）的最低要求。
...
AWS GovCloud （US）使用者必須使用 AWS Secrets Manager 對 FSx for ONTAP 憑證進行驗證。其他所有使用者必須使用檔案系統憑證對 FSx for ONTAP 進行身份驗證。
...
.. 輸入 AWS SSM Parameter ARN 以及 FSx for ONTAP 的 GovCloud 帳戶憑證，或輸入所有其他帳戶的 FSx for ONTAP 檔案系統使用者名稱和密碼，然後選取 *Next* 。
+
對於 GovCloud 帳戶，AWS SSM Parameter ARN 值在 AWS Management Console 的 Parameter Store 中的 Parameter details 下提供，格式如下： `{fsx:{"username":"fsxadmin","password":"pass"}}`。AWS SSM Parameter 類型必須為「Secure String」，名稱格式為 `/netapp/wlmdb/<fsx_filesystemID>`。深入瞭解如何從link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud 文件"^]擷取 SSM 參數。
...
.. 輸入 AWS SSM Parameter ARN 以及 FSx for ONTAP 的 GovCloud 帳戶憑證，或輸入所有其他帳戶的 FSx for ONTAP 檔案系統使用者名稱和密碼，然後選取 *Next* 。
+
AWS SSM 參數類型必須為「Secure String」，名稱格式為 `/netapp/wlmdb/<fsx_filesystemID>`。請以下列格式輸入 AWS SSM Parameter ARN 的值：
`{fsx:{"username":"fsxadmin","password":"pass"}}`。深入瞭解如何從link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud 文件"^]擷取 SSM 參數。
