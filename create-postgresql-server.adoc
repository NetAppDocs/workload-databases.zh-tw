...
summary: 在NetApp Workload Factory for Databases 中建立一個新的 PostgreSQL SQL 伺服器。 
---
= 在NetApp Workload Factory 中建立 PostgreSQL 伺服器
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
在NetApp Workload Factory for Databases 中建立新的 PostgreSQL 伺服器或資料庫主機需要 FSx for ONTAP檔案系統部署和 Active Directory 資源。
...
在從 Workload Factory 建立 PostgreSQL 伺服器之前，請先了解資料庫主機配置的可用儲存部署類型、工作負載工廠操作模式以及完成此操作的要求。
...
+ FSx for ONTAP檔案系統有兩種 PostgreSQL 伺服器部署模型：_高可用性 (HA)_ 或_單一實例_。根據您選擇的 FSx for ONTAP部署模型，為 FSx for ONTAP檔案系統建立不同的資源。
...
你必須有 link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html["授予 _database 主機建立_ 權限"^] 在您的 AWS 帳戶中，在工作負載工廠中建立一個新的資料庫主機。
...
. 在資料庫圖塊中，選擇*部署主機*，然後從選單中選擇*PostgreSQL 伺服器*。
. 選取 * 快速建立 * 。
. 在 * 登陸區 * 下，提供下列項目：
+
...
.. *安全群組*：建立工作負載安全群組是為了允許 PostgreSQL 所需的連接埠和協定。此外，您可以選擇一個或多個安全群組。
...
+
對於 GovCloud 帳戶，請以下列格式輸入憑證的 AWS SSM Parameter ARN 值： `{fsx:{"username":"fsxadmin","password":"pass"}}`。AWS SSM Parameter 類型必須為「Secure String」，名稱格式為 `/netapp/wlmdb/<anyname>`。深入瞭解如何從link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud 文件"^]擷取 SSM 參數。
...
.. *Simple Notification Service*：您可以選擇從下拉式選單中選擇 PostgreSQL 的 SNS 主題，為該組態啟用 Simple Notification Service (SNS)。
+
