...
Workload Factory 現在提供一次性評估，可隨時分析您的 SQL Server 資料庫部署配置 - 新部署、遷移後、故障排除等。您將獲得有助於最佳化部署配置並遵循最佳實務的建議。這是您無需完全投入設定流程即可使用架構完善功能的絕佳機會。您無需在 Workload Factory 中儲存憑證、註冊資料庫資源或設定外部連線即可評估資料庫。
...
=== SQL Server 內部部署資料庫環境成本分析與移轉規劃

BlueXP workload factory for Databases 現在可以偵測、分析並協助您規劃將內部部署 SQL Server 資料庫移轉至 Amazon FSx for NetApp ONTAP 。您可以使用節省成本計算機來估算在雲端執行內部部署 SQL Server 資料庫環境的成本，並檢視將內部部署資料庫環境移轉至雲端的建議。
