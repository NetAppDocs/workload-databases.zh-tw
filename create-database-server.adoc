...
.認證和權限
你必須 link:https://docs.netapp.com/us-en/workload-setup-admin/add-credentials.html["授予 _database 主機建立_ 權限"^] 在您的 AWS 帳戶中，在 Workload Factory 中建立一個新的資料庫主機。
...
+
對於 GovCloud 帳戶，請按以下格式輸入 AWS SSM 參數 ARN 的值：
`{fsx:{"username":"fsxadmin","password":"pass"}}`。

+
該值位於 AWS Management Console 的 Parameter Store 中的 Parameter details 下。AWS SSM Parameter 類型必須為「Secure String」，名稱格式為 `/netapp/wlmdb/<anyname>`。了解更多關於從link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud 文件"^]檢索 SSM 參數的資訊。
...
+
對於 GovCloud 帳戶，請以下列格式輸入 Active Directory 認證的 AWS SSM Parameter ARN 值： `{fsx:{"username":"fsxadmin","password":"pass"}}`。AWS SSM Parameter 類型必須為「Secure String」，名稱格式為 `/netapp/wlmdb/<anyname>`。深入瞭解如何從link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud 文件"^]擷取 SSM 參數。
...
+
對於 GovCloud 帳戶，請以下列格式輸入 FSx for ONTAP 憑證的 AWS SSM Parameter ARN 值： `{fsx:{"username":"fsxadmin","password":"pass"}}`。AWS SSM Parameter 類型必須為「Secure String」，名稱格式為 `/netapp/wlmdb/<anyname>`。深入瞭解如何從link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud 文件"^]擷取 SSM 參數。

+
適用於 ONTAP 檔案系統的全新 FSX 可能會增加 30 分鐘以上的安裝時間。

... *選擇現有的 FSx for ONTAP*：選擇 FSx for ONTAP 名稱，並輸入檔案系統的使用者名稱和密碼。
+
...
.. *安全群組*：工作負載安全群組用於允許節點上 Microsoft SQL 和 Windows 叢集通訊所需的 port 和協定。此外，您可以選擇一個或多個安全群組。
...
+
選用：選擇「使用託管服務帳戶」作為 SQL Server 服務帳戶。如果您的環境使用 MSA（託管服務帳戶）或群組託管服務帳戶 (gMSA)，且密碼管理由 Active Directory 處理，請使用此選項。
...
... 請輸入 Active Directory 網域的使用者名稱和密碼。
+
對於 GovCloud 帳戶，請以下列格式輸入 Active Directory 認證的 AWS SSM Parameter ARN 值： `{fsx:{"username":"fsxadmin","password":"pass"}}`。AWS SSM Parameter 類型必須為「Secure String」，名稱格式為 `/netapp/wlmdb/<anyname>`。深入瞭解如何從link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud 文件"^]擷取 SSM 參數。

... *首選網域控制站*：（選用）輸入要用於加入 Active Directory 的首選網域控制站。
... *首選組織單元路徑*：（選用）在 Active Directory 中輸入要加入的首選組織單元 (OU)。
... *目標 Active Directory 群組*：可選，輸入要將電腦新增至的目標 Active Directory 群組。
...
... * 為 ONTAP * 建立新的 FSX ：輸入使用者名稱和密碼。
+
對於 GovCloud 帳戶，請以下列格式輸入 FSx for ONTAP 憑證的 AWS SSM Parameter ARN 值： `{fsx:{"username":"fsxadmin","password":"pass"}}`。AWS SSM Parameter 類型必須為「Secure String」，名稱格式為 `/netapp/wlmdb/<anyname>`。深入瞭解如何從 link:https://docs.aws.amazon.com/govcloud-us/latest/UserGuide/govcloud-ssm.html["AWS GovCloud 文件"^] 擷取 SSM 參數。
